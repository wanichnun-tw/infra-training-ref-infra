...
locals {
  ...
  eks_master_subnets = slice(data.terraform_remote_state.vpc_state.outputs.vpc.private_subnets,0,3)
  eks_node_subnets = slice(data.terraform_remote_state.vpc_state.outputs.vpc.private_subnets,3,6)
}
...
module "eks" {
  ...
  cluster_addons = {
    ...
    vpc-cni = {
      resolve_conflicts        = "OVERWRITE"
      service_account_role_arn = data.terraform_remote_state.vpc_state.outputs.vpc.vpc_cni_irsa.iam_role_arn
    }
  }
